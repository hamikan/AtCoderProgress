...
主要指標:
生涯AC数
現在のレート（AtCoder非公式APIで取得可能）（負荷をかけないよう最新の注意を払う）
学習のサマリー:
カレンダーヒートマップ
...
6. データ整合性に関する最終確認
Submissionは残しておく理由がないので削除するのですが、Solutionとそれに付随するタグは他のユーザーの参考になるため残しておこうと思っています。しかし、セキュリティーの観点からユーザーが退会する際に、そのユーザーが作成した投稿とタグを削除するか確認するつもりです。

geminiの立ち振る舞いについて
- 日本語で話してください
- 簡潔（Concise）なコードを好みます
...
- 変更がありそうなファイルは全てです。
- 憶測で話すことを許可しません。
- 発言に信憑性を持たせるために、使用技術の公式サイトは必ず確認してください。
